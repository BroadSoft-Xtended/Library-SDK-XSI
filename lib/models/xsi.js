    ...
    var actionRequest = function(opts) {
      ...
      if (!agent) {
        if(client.jsessionid) {
          debug.debug('using jsessionid : '+client.jsessionid);
          opts.cookie = 'JSESSIONID='+client.jsessionid;
        } else if (token) {
          debug.debug('using token : '+token);
          opts.broadworksSSO = token;
        } else {
          debug.debug('using basic : '+user);
          opts.basic = {
            user: user,
            password: password
          }
      ...
      return requestObj.send(opts).then(function(res) {
        ...
        if(setCookie) {
          ...
          client.jsessionid = getCookie(setCookie, 'JSESSIONID') || client.jsessionid;
